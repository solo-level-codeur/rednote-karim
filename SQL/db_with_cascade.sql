-- ----------------------------------------------------------
-- Script MYSQL avec DELETE CASCADE pour système de gestion de projets
-- ----------------------------------------------------------
...
-- =====================================================
-- CLÉS ÉTRANGÈRES AVEC DELETE CASCADE
-- =====================================================

-- Users (pas de cascade sur les rôles pour éviter la suppression accidentelle)
ALTER TABLE users
  ADD CONSTRAINT users_id_roles_FK FOREIGN KEY (id_roles)
  REFERENCES roles (id);

-- Projects (pas de cascade sur le propriétaire)
ALTER TABLE projects
  ADD CONSTRAINT projects_id_owner_FK FOREIGN KEY (id_owner)
  REFERENCES users (id_users);

-- Tasks (cascade sur projet, pas sur utilisateur assigné)
ALTER TABLE tasks
...
-- Notes (cascade sur projet, pas sur utilisateur)
ALTER TABLE notes
...
-- Documents (pas de cascade sur utilisateur)
ALTER TABLE documents
  ADD CONSTRAINT documents_id_uploaded_by_FK FOREIGN KEY (id_uploaded_by)
  REFERENCES users (id_users);

-- Comments (cascade sur note, pas sur utilisateur)
...
-- Role Permissions (cascade sur rôle et permission)
...
-- User Skills (cascade sur skill, pas sur utilisateur)
...
-- Project Members (cascade sur projet, pas sur utilisateur)
...
-- Project Tags (cascade sur projet et tag)
...
-- Note Tags (cascade sur note et tag)
...
-- Note Documents (cascade sur document et note)
...
-- Note Shares (cascade sur note, pas sur utilisateur)
